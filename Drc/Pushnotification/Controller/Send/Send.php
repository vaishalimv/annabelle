...
namespace Drc\Pushnotification\Controller\Send;
    ...
    public function execute()
    {
    	...
    	if($pid)
    	{
			...
			/* ios devices */			
	      	...
	      	$this->sendNotification($storeManager,$baseurl,$server_key,$objectManager,$Collection,$iosdevices,'i');
			...
			$this->sendNotification($storeManager,$baseurl,$server_key,$objectManager,$Collection,$andevices,'a');			

			$this->_messageManager->addSuccess(__("Success"));
			$this->_redirect('admin/pushnotification/pushnotification/index');
	        return true;
	    }else{
			...
			if(!empty($cronCol->getData()))
			{
				foreach ($cronCol as $ck => $cv) {
					if( $ct==strtotime($cv->getDateTime()) ){
				      	...
				      	$this->CronSendNotification($storeManager,$baseurl,$server_key,$objectManager,$colldata,$cronCol,$iosdevices,'i');
				      	...
				      	$this->CronSendNotification($storeManager,$baseurl,$server_key,$objectManager,$colldata,$cronCol,$andevices,'a');
